...
# 模型类表的设计还是属于业务逻辑所以放在ihome文件夹下, 但是并不是api 所以和api包同一层级
...
# 根据　house_id和关联表 查到facility_id, 再查到house拥有的facility_id
# select facility_id from ih_house_facility where house_id = 1; ==> []
# select * from ih_facility where id in facility_id
